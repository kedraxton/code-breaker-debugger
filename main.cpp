...
//added the cstdlib and took out "stdafx.h"
#include <cstdlib>
#include <iostream>
#include <string>
...
int main()
{
    ...
    //changed this to class and not int, foo is used as a placeholder for a value
    //that can change
    class foo;
    int vowels = 0;
    int consonants = 0;
    ...
    int checkSum = 0;
    //changed this from int to class, same reason as foo above
    class bar;
    ...
    for (int i = 0; i < textToAnalyze.length(); ++i)
    {
        if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
            ...
            textToAnalyze[i] == 'U')
        {
            //changed this from -- to ++ because we need to add the number of
            //vowels in the statement
            ++vowels;
        }
